...
export function AddReviewForm() {
  ...
  const form = useForm<InsertReview>({
    resolver: zodResolver(insertReviewSchema.extend({
      name: insertReviewSchema.shape.name.min(4, t("reviews_name_min")),
      content: insertReviewSchema.shape.content.min(175, t("reviews_content_min")),
    })),
    defaultValues: {
      name: "",
      content: "",
  ...
  return (
    <div className="w-full max-w-xl mx-auto p-8 bg-neutral-900/30 border border-white/5 rounded-3xl mt-12">
      <h3 className="text-2xl font-bold text-white mb-6 text-center">{t("reviews_add")}</h3>
      
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="space-y-2">
          <label className="text-sm font-medium text-white/70">{t("reviews_name_label")}</label>
          <input
            {...form.register("name")}
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-white/70">{t("reviews_rating")}</label>
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-white/70">{t("reviews_review_label")}</label>
          <textarea
            ...
            placeholder={t("reviews_review_placeholder")}
          />
        ...
        >
          {createReview.isPending ? t("reviews_sending") : t("reviews_submit")}
        </button>
      </form>
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent className="bg-neutral-900 border-white/10 text-white max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">{t("reviews_confirm_title")}</DialogTitle>
            <DialogDescription className="text-white/70 text-lg mt-4">
              {t("reviews_confirm_desc")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2 text-sm text-red-400/80">
              <p>• {t("reviews_warn_1")}</p>
              <p>• {t("reviews_warn_2")}</p>
            </div>

            <p className="font-bold">{t("reviews_confirm_question")}</p>
          ...
          <DialogFooter className="flex flex-row gap-4 sm:justify-end">
            ...
            >
              <X className="w-4 h-4" /> {t("reviews_no")}
            </Button>
            <Button 
            ...
            >
              <Check className="w-4 h-4" /> {t("reviews_confirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
